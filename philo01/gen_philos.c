	...
	gen_philos(t_rule *rule)
{
	...
	while (++i < rule->number_of_philos)
	{
		...
		rule->philo[i].left_fork = rule->philo[i].philo_id % rule->number_of_philos;
		rule->philo[i].right_fork = (rule->philo[i].philo_id - 1) % rule->number_of_philos;
